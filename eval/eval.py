...
def evaluate():
    ...
    predicted = getIdsNLabels(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "../output_data/predictions.csv"), 2)
    ...
    try:
        ...
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "../output_data/evaluation_results.csv"),
                  'w', encoding="utf-8") as new_file:
            new_file.write("Accuracy: %1.2f" % getAccuracy(y_true, y_predicted) + "\n"
                           + "(macro) Precision: %1.2f" % getPrecision(y_true, y_predicted) + "\n"
                           + "(macro) Recall: %1.2f" % getRecall(y_true, y_predicted))

    except ValueError as e:
        import sys
        sys.stderr.write(str(e) + "\n")
        sys.stderr.write(
            "Please check your output file, e.g. does it contain predictions for every claim ID in the test set?")
